require(["jqwidgets", "helper"], function () {
    ...
    var table01 = "#tableRegRoles";

    $(table01).CreateTable({
        query: 'GET_ROLES_FULL',
        items: {
            BUSCAR: function () {
                return $('input#_buscar').val() || '';
            }
        },
        hiddens: ['FEC_CREAC', 'USU_CREAC', 'HOST_CREAC', 'C_EMPRESA', 'HOST_MODIF', 'DESCRIPCION', 'TIPO_ROL'],
        sortColumns: {
            FEC_MODIF: 'DESC'
        },
        columns: {
            C_ROL: { text: 'Código', width: '120', align: 'center', cellsalign: 'center' },
            ...
            FLAG_ESTADO: {
                text: 'Estado', width: '100', align: 'center', cellsalign: 'center',
            ...
            FEC_MODIF: { text: 'Fecha modificación', width: '180', align: 'center', cellsalign: 'center', cellsformat: 'dd/MM/yyyy HH:mm:ss' },
            DESCRIPCION_PARAMETRO: { text: 'Tipo de rol', width: '150', align: 'center', cellsalign: 'center' },
            Accion: {
                text: 'Acciones', width: '150', align: 'center', cellsalign: 'center', cellsRenderer: function (row, column, value, rowData) {
                    var _editar = '<a href="/seguridad/Configurar/RolesRegistro/' + rowData["C_ROL"] + '" data-id="' + rowData["C_ROL"] + '" class="btn-editar"><i class="fa fa-pencil-square" aria-hidden="true"></i> Editar</a>';
    ...
    $('form[name=filtrosRegRoles]').submit(function () {
        $(table01).jqxDataTable('render');
    });
