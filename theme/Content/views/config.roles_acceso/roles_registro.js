require(['jqwidgets', 'helper', 'extras'], function () {
    ...
    const table01 = "#tableRegUsuarios";
    let urlActionBack = '';

    if ($.solver.basePath == '/administracion') urlActionBack = $.solver.basePath + '/Mantenimiento/RolesAcceso';
    if ($.solver.basePath == '/seguridad') urlActionBack = $.solver.basePath + '/Configurar/RolesAcceso';
    ...
    var actionsButtons = function (controls) {
        ...
        $('button.btn-add-user').click(function () {
            $.GetData({
                ...
                uriData: '/Seguridad/Configurar/Rolesbuscarusuario',
                location: 'float',
                size: 'large',
                type: 'GET',
                isPage: true,
    ...
    var checkCountDetail = function () {
        ...
        if (contar_opciones == 0 && continuar == true) {
            require(["alertify"], function (alertify) {
                ...
                alertify.error('El rol debe tener como minimo un Acceso Habilitado.');
            });
            continuar = false;
        };
    ...
    $('form[name=frmRegistroRol]').ValidForm({
        ...
        onDone: function (form, controls) {
            ...
            $(controls.TIPO_ROL).change(function () {
                ...
                $('#editerLimit').addClass('d-none');
                if ($(controls.TIPO_ROL).val() == '09708') {
                    $('#editerLimit').removeClass('d-none');
                };
                if ($(controls.TIPO_ROL).val() == '09708' || $(controls.TIPO_ROL).val() == '09709') {
                    $('#contact-tab').parent().hide();
                } else {
            ...
            //valores por defecto
            if ($(controls.LIMITE_USUARIOS).val() == '') $(controls.LIMITE_USUARIOS).val(0);
            if ($(controls.LIMITE_OPERACIONES).val() == '') $(controls.LIMITE_OPERACIONES).val(0);
            if ($(controls.LIMITE_ESTABLECIMIENTOS).val() == '') $(controls.LIMITE_ESTABLECIMIENTOS).val(0);
            if ($(controls.LIMITE_ALMACENES).val() == '') $(controls.LIMITE_ALMACENES).val(0);
            if ($(controls.PRECIO_VENTA).val() == '') $(controls.PRECIO_VENTA).val(0);

            if ($.solver.basePath == '/administracion') {
                $(controls.TIPO_ROL).val('09708').trigger('change');
                $(controls.TIPO_ROL).attr('readonly', 'readonly').css({ 'pointer-events': 'none' });
            };

            $(controls.C_EMPRESA).val('E000000001').trigger('change');
            $(controls.C_EMPRESA).attr('readonly', 'readonly').css({ 'pointer-events': 'none' });
        ...
        onReady: function () {
            //document.location = $.solver.baseUrl + '/Configurar/RolesAcceso/';
            if ($.solver.basePath == '/administracion') document.location = $.solver.basePath + '/Mantenimiento/RolesAcceso';
            if ($.solver.basePath == '/seguridad') document.location = $.solver.basePath + '/Configurar/RolesAcceso';
        },
    ...
    $('a#lnkBack').attr('href', urlActionBack);
