    ...
    require(["alertify"], function (alertify) {
        ...
        const fnCrearTabla = function () {
            $(tblProductos).CreateGrid({
                ...
                sortdirection: 'DESC',
                hiddens: ['CENTRO', 'C_UNIDAD_NEGOCIO_REF', 'AFECTACION_IGV', 'DETRACCION', 'TIPO_UNIDAD', ($.solver.basePath == '/restaurant' ? '' : 'NOMBRE_COCINA')],
                columns: {
                    '_rowNum': {
                        text: '#',
                        width: '30',
        ...
        $('form[name=filtrosRegProductos]').ValidForm({
            ...
            onDone: function (_, controls) {
                ...
                //    $(controls.C_PARAMETRO_GENERAL_TIPO_PRODUCTO).val('07228');
                //    $(controls.C_PARAMETRO_GENERAL_TIPO_PRODUCTO).attr('readonly', 'readonly').css({ 'pointer-events': 'none' });
                //}
                //if ($.solver.basePath == '/puntoventa' && $.solver.basePath == '/restaurant') {
                //    $(controls.C_PARAMETRO_GENERAL_TIPO_PRODUCTO).val('07228');
                //    $(controls.C_PARAMETRO_GENERAL_TIPO_PRODUCTO).attr('readonly', 'readonly').css({ 'pointer-events': 'none' });
                //}
                //if ($.solver.basePath == '/restaurant') {
                //    $(controls.C_PARAMETRO_GENERAL_TIPO_PRODUCTO).val('09994');
                //    $(controls.C_PARAMETRO_GENERAL_TIPO_PRODUCTO).attr('readonly', 'readonly').css({ 'pointer-events': 'none' });
                //}
